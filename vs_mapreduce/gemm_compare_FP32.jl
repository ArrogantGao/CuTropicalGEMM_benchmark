...
df_gemm = CSV.read("cutropicalgemm_FP32.csv", DataFrame)
df_mapreduce = CSV.read("mapreduce_FP32.csv", DataFrame)
...
# briefly, we can get a rule: if log2(M) > 15, (log2(N) + log2(K)) < 7, use mapreduce
# why?